...
def load_multi_race_laps(race_driver_list):
    ...
    # แต่ละ race มี Compound_*/TrackStatus_* one-hot ไม่เหมือนกัน (เช่น สนามแห้งไม่มี
    # Compound_INTERMEDIATE) พอ concat แล้ว pandas เติม NaN ให้คอลัมน์ที่ frame หนึ่งมีแต่อีก
    # frame ไม่มี ซึ่งเปลี่ยน dtype จาก bool เป็น object — ทำให้ train_model_advanced.py
    # คัดคอลัมน์เหล่านี้ทิ้งไปเงียบๆ ตอนกรองด้วย select_dtypes (ทั้งที่ตั้งใจเก็บ bool ไว้แล้ว)
    # เติม NaN เป็น False แล้วบังคับกลับเป็น bool ก่อน เพื่อให้ชนิดยาง/สถานะสนามไม่หายจากโมเดล
    onehot_cols = [c for c in combined.columns if c.startswith(("Compound_", "TrackStatus_"))]
    if onehot_cols:
        combined[onehot_cols] = combined[onehot_cols].fillna(False).astype(bool)
